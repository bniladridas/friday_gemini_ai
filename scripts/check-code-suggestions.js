...
Suggestions should be actionable and include line numbers.
`;
...
function parseCodeSuggestions(analysis) {
  ...
  for (const block of diffBlocks) {
    ...
    if (lines.length > 0) {
      ...
      if (hunkMatch) {
        const line = parseInt(hunkMatch[1]);
        const suggestionLines = lines.slice(2).filter(l => l.startsWith('+')).map(l => l.slice(1));
        const suggestion = suggestionLines.join('\n');
        if (suggestion) {
          suggestions.push({ file: filePath, line, suggestion });
        }
      }
    }
  }
